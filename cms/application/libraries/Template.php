...
class Template
{
    ...
    {
        self::_build($view, $data = array(), $return = FALSE);
    }    // --------------------------------------------------------------------
     ...
     * @return    void
     */
    public function _build($view, $data = array(), $return = FALSE)
    {
    ...
    {
        $this->_breadcrumbs[] = array('name' => $name,
            'uri' => $uri);
    ...
    private function _find_view($view, array $data, $parse_view = TRUE)
    {
        ...
        if(!empty($this->_theme)) {
            foreach($this->_theme_locations as $location) {
                $theme_views = array($this->_theme . '/views/modules/' . $this->_module . '/' . $view,
